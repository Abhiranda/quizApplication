...
const option_box = document.querySelector(".option_list");
...
continue_btn.onclick = () => {
    ...
    showQuetions(0); //calling showQuetion function
...
restart_quiz.onclick = ()=> {
    ...
    userScore = 0;
    widthValue = 0;
    showQuetions(que_count); //calling showQuestion function
...
next_btn.onclick = () => {
    if(que_count < question.length-1) { // if question count is less than total question length
        ...
        que_numb++; //increment the que-numb value
        showQuetions(que_count); //calling showQuestion function
        queCounter(que_numb); //passing que_numb value to queCounter
        clearInterval(counter); //clear counter
        clearInterval(counterLine); // clear counterLine
...
function showQuestion(index){
    ...
    + '</span></div>'
    que_text.innerHTML = que_tag; //adding new span tag inside que-tag
...
function optionSelected(answer){
    clearInterval(counter); //clear counter
    clearInterval(counterLine); //clear counterLine
    let userAns = questions [que_count].answer; //getting correct answer from array
    const allOptions =option_list.children.length; //getting all option items

    if(userAns == correctAns){ // if user selected option equals to array's crrect answer
        userScore += 1; //upgrading score value with 1
        answer.classList.add("correct"); //adding green color to correct selected option 
        answer.insertAdjacentHTML("beforeend",tickIcontag); //adding tick icon to correct selected option
        console.log("Correct Answer");
        console.log("Youre correct answers = " +userScore);
    } else {
        ...
        for(i=0; i < allOptions; i++) {
            ...
            { //if there is an option which is matched to an array answer
                ...
                ("beforeend",tickIconTag); //adding tick icon to the matched option
...
function showResult(){
    ...
    if (userScore > 3){ //if user score more than 3
        //creating a new span tag and passing the user score number and total quesion number
        let scoreTag = '<span>Congrats!, yo got <p>'+ userScore+'</p> out of <p>'+questions.length + '</p></span>';
        ...
        if(time<0){ //if timer is less than 0
            ...
            for(i=0; i<allOptions;i++){
                if(option_list.children[i].textContent == correctAns){ //if there is an option which is matched to an array answer
                    option_list.children[i].setAttribute("class","option correct");//adding green color to matched option
                    option_list.children[i].insertAdjacentHTML("beforeend",tickIconTag); //adding tick icon to the matched option
                    console.log("Time Off: Auto seleceted correct answer");
                }
            }
...
function queCounter(index){
    //creating a new span tag and passing the question number and total qustion
